...
%%%=== DIY宏包添加区域  === %%%
\usepackage{multirow}     %用于给表格设置多行或多列合并
\usepackage{amsmath, bm}  %AMS 数学公式 宏包
\usepackage{amssymb}      %AMS 数学符号 宏包
\usepackage{shorttoc}     %设置显示章节标题的级别

%%%=== 参考文献 === %%%
%\bibliographystyle{unsrt}        % 参考文献样式,  plain,unsrt,alpha,abbrv 等等
\usepackage[numbers]{gbt7714}     %中文国标样式
...
%-----------------------------------------------------------------------------
\include{includefile/frontmatter}    % 加入英文封面, 论文创新点, 等等目录之前的部分.
\frontmatter
...
\pagestyle{oldplain}
\tableofcontents
%\shorttableofcontents{目\qquad 录}{2}  %如果要设置只显示3级目录，则需要先用\tableofcontents编译后，然后注释掉，换成这句话编译
...
%\bibliography{report}    %report.bib是用来存放参考文献的文件，需要把thebibliography注释掉(推荐这种方法，比较方便)

\backmatter

\include{includefile/backmatter} %%%致谢、作品列表 ．
